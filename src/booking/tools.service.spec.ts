...
describe("ToolsService", () => {
  ...
  const input = { name: "J", email: "j@x.com", phone: "5", vehicle: "v", service: "s", date: "2026-06-23", time: "10:00" };
  ...
  describe("rescheduleAppointment", () => {
    it("rejects when new slot is full", async () => {
      findFirst.mockResolvedValue({ id: "appt_1", googleEventId: "evt_1" });
      getOpenSlots.mockResolvedValue(["09:00"]);
      const r = await svc.rescheduleAppointment({ identifier: "AC-1234", newDate: "2026-06-24", newTime: "10:00" });
      expect(r.ok).toBe(false);
    });
