...
describe("CalendarBookingService", () => {
  ...
  it("creates event with correct title, guest, and returns id", async () => {
    ...
    expect(arg.requestBody.summary).toBe("Brake check — John Smith (Toyota Corolla 2018)");
  ...
  it("deleteEvent calls events.delete with correct params", async () => {
    ...
    expect(deleteFn).toHaveBeenCalledWith(expect.objectContaining({ eventId: "evt_1", sendUpdates: "all" }));
  });

  it("moveEvent calls events.patch with new times", async () => {
    await svc.moveEvent("evt_1", "2026-06-24", "14:00");
    expect(patch).toHaveBeenCalledWith(expect.objectContaining({ eventId: "evt_1", sendUpdates: "all" }));
  });
});
